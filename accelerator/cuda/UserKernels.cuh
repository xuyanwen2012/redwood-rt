...
__global__ void CudaBarnesHut(const int* u_leaf_indices,
  ...
  for (; it < how_many_times_loop; ++it) {
    ...
    const float aggregate = BlockReduce(temp_storage.reduce).Sum(thread_value);
...
__global__ void FindMinDistWarp6(
  ...
  for (int ln = warp_id; ln < num_active_leafs; ln += 32) {
    ...
    for (int group = 0; group < LeafSize; group += 32) {
      const int group_id = group / 32;

      // kernel function
      const auto dist = KernelFuncKnn(
          u_lnt_data[leaf_node_uid * LeafSize + group + lane_id], q);
...
//
template <int LeafSize>
__global__ void CudaNn(const int* u_leaf_indices,  /* batch_size */
                       const Point4F* u_q_points,  /* batch_size */
                       const int num_active_leafs, /* how many collected */
                       float* outs,                /* batch_size */
                       const Point4F* u_lnt_data,  /**/
  ...
  for (; it < how_many_times_loop; ++it) {
    ...
    for (int i = 0; i < items_per_thread; ++i) {
      thread_value[i] = KernelFuncKnn(thread_data[i], q);
    }

    const auto aggregate = WarpReduce(temp_storage.reduce[warp_id])
                               .Reduce(thread_data, cub::Min());

    // if (tid == 0) {
    // outs[tid] = min(out[0], aggregate);
    // }
  }
}
...
//     // Load entire leaf node at location (given 'leaf_id_to_load')
//     WarpLoad(temp_storage.load[warp_id])
//         .Load(u_lnt_data + my_leaf_id_to_load * leaf_size_i_want,
//         thread_data);
...
//   const auto my_leaf_id_to_load = u_leaf_indices[tid];
//   const auto q = u_q_points[tid];

//   float my_min = std::numeric_limits<float>::max();

//   for (int i = 0; i < LeafSize; ++i) {
//     const auto dist = KernelFuncKnn(thread_data[i], q);
//     auto my_min = WarpReduce(temp_storage[warp_id]).Reduce(dist, cub::Min());
//   }
