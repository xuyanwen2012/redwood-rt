...
// template <typename DataT, typename QueryT, typename ResultT>
// __global__ void NaiveProcessBhBuffer(const QueryT* query_points,
//                                      const int* query_idx, const int*
//                                      leaf_idx, const DataT* leaf_node_table,
//                                      ResultT* out, const int num, const int
//                                      leaf_node_size) {
//   const auto tid = blockIdx.x * blockDim.x + threadIdx.x;
...
//   for (int i = 0; i < leaf_node_size; ++i) {
//     const auto dist =
//         KernelFunc(leaf_node_table[leaf_id * leaf_node_size + i],
//         query_point);
