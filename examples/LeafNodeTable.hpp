...
void AllocateLeafNodeTable(const int num_leaf_nodes, const int leaf_size,
...
void FreeLeafNodeTalbe() {
  redwood::UsmFree(lnt.u_data);
  if (lnt.use_sizes) redwood::UsmFree(lnt.u_sizes);
}

_NODISCARD Point4F* LntDataAddr() { return lnt.u_data; }

_NODISCARD const Point4F* LntDataAddrAt(const int node_idx) {
  return lnt.u_data + node_idx * lnt.leaf_size;
}

_NODISCARD int* LntSizesAddr() { return lnt.u_sizes; }
