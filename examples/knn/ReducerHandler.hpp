...
inline void Init(const int batch_size) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    // K = 32
    result_addr[i].Alloc(batch_size, 32);
...
inline void DebugCpuReduction(const Buffer& buf, const dist::Euclidean functor,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    const auto addr =
        reinterpret_cast<KnnSet<float, 32>*>(results.GetAddrAt(i));
...
inline void LaunchAsyncWorkQueue(const int stream_id) {
  ...
  //                          result_addr[stream_id].underlying_dat,
  //                          lnt_base_addr, stored_max_leaf_size, stream_id);
}
}  // namespace rdc
