...
struct BarnesHandler {
  ...
  std::array<ResultT*, kNumStreams> usm_result;
  std::array<DataT, kNumStreams> h_query;
};

using MyBarnesHandler = BarnesHandler<Point4F, float>;

template <typename DataT, typename ResultT>
struct BarnesReducer
    : ReducerBase<BarnesReducer<DataT, ResultT>, DataT, ResultT> {
