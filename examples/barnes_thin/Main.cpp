...
*/
  void StartQueryPb(const Point4F q, int root,
                    BlockStack* stack) {
    stats_.leaf_node_reduced = 0;
    stats_.branch_node_reduced = 0;
    my_q_ = q;
  ...
  // host_results_;}
 private:
  _NODISCARD static float ComputeThetaValue(int node,
                                            const Point4F& pos) {
    //   std::cout << "compute thetavalue" <<std::endl;
    const auto com = oct::center_of_mass[node];
    ...
    return (oct::bounding_boxes[node].max.data[0]- oct::bounding_boxes[node].min.data[0]) / norm;
    //return node->bounding_box.dimension.data[0] / norm;
  }

  void TraversePointBlocking(int cur, BlockStack* stack,
                             int level) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (oct::nodes[cur].IsLeaf()) {
        if (oct::range[cur].first == oct::range[cur].second) {
          return;
        }

        // ------------------------------------------------------------
        const auto leaf_addr = rdc::LntDataAddrAt(oct::nodes[cur].uid);
        ...
        // ------------------------------------------------------------
        host_result_ += functor(query_node.second, oct::center_of_mass[cur]);
        // final_results[query_node.q_idx] += functor(query_node.query,
    ...
    if (next_block->size() > 0) {
      ...
      for (const auto child : oct::nodes[cur].children) {
        if (child != -1) TraversePointBlocking(child, stack, level + 1);
      }
    }
  }
  /*
