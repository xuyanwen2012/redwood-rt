...
libnames = ('libcuda.so.1', 'libcuda.dylib', 'cuda.dll')
...
def get_cuda_device_specs() -> List[Dict[str, Any]]:
    ...
    for i in range(num_gpus.value):
        ...
        if cuda.cuDeviceComputeCapability(ctypes.byref(cc_major), ctypes.byref(cc_minor), device) == CUDA_SUCCESS:
            ...
            spec.update(architecture=SEMVER_TO_ARCH.get(
        ...
        if result != CUDA_SUCCESS:
            ...
            try:
                ...
                result = cuda.cuMemGetInfo(ctypes.byref(
                    free_mem), ctypes.byref(total_mem))
            if result == CUDA_SUCCESS:
                spec.update(
